...
class MokeyAuthenticationBackend(OIDCAuthenticationBackend):
    def _sync_groups(self, user, groups):
        ...
        for group_name in groups:
            ...
            if group_name == settings.MOKEY_OIDC_PI_GROUP:
                is_pi = True
    ...
    def verify_claims(self, claims):
        verified = super(MokeyAuthenticationBackend, self).verify_claims(claims)

        if len(settings.MOKEY_OIDC_ALLOWED_GROUPS) == 0 and len(settings.MOKEY_OIDC_DENY_GROUPS) == 0:
            return verified and True

        groups = self._parse_groups_from_claims(claims)

        if len(settings.MOKEY_OIDC_ALLOWED_GROUPS) > 0:
            for g in settings.MOKEY_OIDC_ALLOWED_GROUPS:
                if g not in groups:
                    return False

        if len(settings.MOKEY_OIDC_DENY_GROUPS) > 0:
            for g in settings.MOKEY_OIDC_DENY_GROUPS:
                if g in groups:
                    return False
