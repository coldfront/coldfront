...
class ProjectOpenLdapTestCase(TestCase):
    ...
    def _is_user_in_project_nonArchived_posixGroup(self, user: User, project: Project):
    ...
    def _is_user_in_project_archived_posixGroup(self, user: User, project: Project):
        dn = self._get_project_archived_posixGroup_dn(project)
        self.assertTrue(self._does_entry_exist(dn, "posixGroup"))
        return user.username in self._get_entry_memberUid(dn)

    def _does_project_nonArchived_ou_exist(self, project):
        return self._does_entry_exist(self._get_project_nonArchived_ou_dn(project), "organizationalUnit")

    def _does_project_nonArchived_posixGroup_exist(self, project):
        return self._does_entry_exist(self._get_project_nonArchived_posixGroup_dn(project), "posixGroup")

    def _does_project_archived_ou_exist(self, project):
        return self._does_entry_exist(self._get_project_archived_ou_dn(project), "organizationalUnit")

    def _does_project_archived_posixGroup_exist(self, project):
        return self._does_entry_exist(self._get_project_archived_posixGroup_dn(project), "posixGroup")
