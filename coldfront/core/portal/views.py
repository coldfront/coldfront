...
def center_summary(request):
    ...
    context["grants_total"] = intcomma(
        int(sum((float(x) for x in list(Grant.objects.values_list("total_amount_awarded", flat=True)))))
    )
    context["grants_total_pi_only"] = intcomma(
        int(
            sum(
                (float(x) for x in list(Grant.objects.filter(role="PI").values_list("total_amount_awarded", flat=True)))
            )
        )
    )
    context["grants_total_copi_only"] = intcomma(
        int(
            sum(
                (
                    float(x)
                    for x in list(Grant.objects.filter(role="CoPI").values_list("total_amount_awarded", flat=True))
                )
            )
        )
    )
    context["grants_total_sp_only"] = intcomma(
        int(
            sum(
                (float(x) for x in list(Grant.objects.filter(role="SP").values_list("total_amount_awarded", flat=True)))
            )
        )
    )
